...
无论是个人用户需要远程访问家中的 NAS 和智能设备，还是企业团队需要在全球各地安全地协作，构建一个 安全、高效且易于管理的网络环境变得至关重要。传统的 VPN 解决方案往往伴随着复杂的配置、性能瓶颈以及中心化服务器的潜在单点故障问题。

比如最常见的需求就是，公司有一个内网办公环境，当外出办公时，也希望个人电脑能够接入办公网络。 因为外网的机器和内网的机器不能互联，所以一般会有一个中心服务器， 所有的子节点都和中心服务器相连，然后中心服务器转发所有的流量

<img src="./.assets/Tailscale/640.png" alt="img" style="zoom:67%;" />

这样做的缺点显而易见，首先是中心服务器（hub）会成为`瓶颈`。 

中央网关可能靠近用户，也可能不靠近用户，从而导致更高的延迟。由于流量是集中的，它也可能成为瓶颈，进一步降低连接速度。

<img src="./.assets/Tailscale/640-20260310112021134.png" alt="img" style="zoom:67%;" />

每个设备都直接连接到另一个设备，从而降低了延迟。

### WireGuard

那么，能不能让节点间直接互联呢？ 这就是 Mesh VPN，其实现就是 Google Wireguard

<img src=".assets/Tailscale/pQ8X8e.jpg" alt="img" style="zoom:67%;" />
...
WireGuard 相比于传统 VPN 的核心优势是没有 VPN 网关，所有节点之间都可以点对点（P2P）连接，也就是全互联模式（Full Mesh），效率更高，速度更快，成本更低

可以将 Tailscale 看成是更为易用、功能更完善的 WireGuard

### Tailscale

Tailscale（<https://github.com/tailscale/tailscale>） 基于 Google 的 WireGuard 协议，并在此基础上构建了一层智能控制平面，极大地简化了网络配置和管理。Tailscale 不仅仅是一个 VPN，它更像是一个智能的、无需额外硬件的网络层，将你的所有设备，无论身在何处，都安全地连接在一起，仿佛它们都在同一个局域网中。
...
强大的 访问控制列表（ACLs） 功能。可以通过策略文件，精确地定义哪些用户可以访问哪些设备或服务

- MagicDNS

MagicDNS： Tailscale 会自动为网络中的设备分配一个易于记忆的域名，例如 `my-server.alpha-quant.tech`。无需记住 IP 地址，可以直接通过域名访问设备，极大地提升了使用的便利性。

- Taildrop

Taildrop： 这是一项类似 AirDrop 的功能，允许你在 Tailscale 网络中的设备之间安全地传输文件。它利用了 Tailscale 的加密隧道，确保文件传输的私密性和安全性。
...
<img src=".assets/Tailscale/4c2e6087bc4646031a121f12d0de287f.jpeg" alt="img" style="zoom: 67%;" />
...
<img src="./.assets/Tailscale/MrF6yn.png" alt="img" style="zoom:50%;" />
...
对等中继（Peer Relays）功能，无需任何额外部署，能使用已经在 Tailscale 网络中的设备充当中继服务

打开节点的 UDP 40000 端口（在路由器上设置端口转发，或者打开 DMZ 服务器）
