将 Ansible 视为一门编程语言（DSL）的现在，可以发现在 Ansible 文档中用 Roles 表示“模块”这个概念，用 import_tasks、include_tasks 等表示 import 这个概念

![image-20251120133454251](./.assets/AnsibleRoles/image-20251120133454251.png)
...
Ansible 中已内置了一个命令行工具 ansible-galaxy 快速创建 role 的目录，减轻工作量

```
ansible-galaxy init flink
```

Ansible Galaxy 也有一个在线社区 Galaxy[<https://galaxy.ansible.com/home>]，上面有开发者分享的各种已经开发好的 roles。方便搜索现成的 role 下载，也可以上传自己开发的 role 到 Galaxy

```
ansible-galaxy install username.role_name
```

使用 `type: git` 安装集合时，version 键可以引用分支或 git commit-hash 对象（提交或标签）。例如：
