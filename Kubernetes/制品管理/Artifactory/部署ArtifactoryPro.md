...
```bash
docker pull releases-docker.jfrog.io/jfrog/artifactory⁠-⁠pro:6.9.0
# docker pull docker.bintray.io/jfrog/artifactory⁠-⁠pro:6.9.0

docker pull releases-docker.jfrog.io/jfrog/nginx-artifactory-pro:latest
# docker pull docker.bintray.io/jfrog/nginx-artifactory-pro:latest
```
...
docker run \
 ...
 --rm \
 -p 18081:8081 \
 -p 18082:8082 releases-docker.jfrog.io/jfrog/artifactory-pro:6.9.0
```
...
```bash
docker pull releases-docker.jfrog.io/jfrog/artifactory-pro:7.27.10
# docker pull docker.bintray.io/jfrog/artifactory-pro:7.27.10

docker pull releases-docker.jfrog.io/jfrog/nginx-artifactory-pro:7.27.10

docker pull releases-docker.jfrog.io/postgres:13.2-alpine
```
...
使用 `./config.sh` 进行部署，建议是带 postgresql 的实例

```yaml
version: '3'
services:
  artifactory:
    image: ${DOCKER_REGISTRY}/artifactory-pro:${ARTIFACTORY_VERSION}
...
DOCKER_REGISTRY=dockerhub.bigquant.ai:5000

## Version of artifactory to install
ARTIFACTORY_VERSION=6.9.0-hack
...
ROOT_DATA_DIR=/bqmnt/data/artifactory/data
...
```bash
# 生成证书
openssl req -x509 -nodes -days 700 -newkey rsa:2048 -keyout repos-tls.key -out repos-ca.crt -subj "/CN=repos.bigquant.ai"
```
...
```txt
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
```
...
```txt
-----BEGIN CERTIFICATE-----

T3Ktjxz+3WoqSeRwQG/y
-----END CERTIFICATE-----
```
...
```bash
kubectl -n artifactory create secret tls artifactory-https-secret \
    --key repos-tls.key \
    --cert repos-ca.crt --dry-run -o yaml |kubectl apply  -f -
```

### 生成 master key 和 join key

在正式生产环境部署需要固定这两个 key，方便后期维护集群
...
kubectl create secret generic bigquant-masterkey-secret -n artifactory --from-literal=master-key=${MASTER_KEY}
```
...
kubectl create secret generic bigquant-joinkey-secret -n artifactory --from-literal=join-key=${JOIN_KEY}
```
...
```bash
#############################
chmod 777 /mnt/sda4/artifactory/pg_data
chmod 777 /mnt/sda4/artifactory/app_data
```
...
apiVersion: v1
metadata:
  name: data-bigquant-postgresql-pv
  labels:
...
spec:
  capacity:
    storage: 50Gi
  hostPath:
    path: /mnt/sda4/artifactory/pg_data
...
apiVersion: v1
metadata:
  name: artifactory-volume-bigquant-artifactory-pv
  labels:
    app: artifactory
    release: bigquant
    role: artifactory
spec:
  capacity:
    storage: 250Gi
  hostPath:
    path: /mnt/sda4/artifactory/app_data
    type: DirectoryOrCreate
  accessModes:
    - ReadWriteOnce
  volumeMode: Filesystem
...
初始密码 admin/password 改一下密码 -> admin / Bigquant123! 然后就不用管了，可以开始 hack

```bash
############################ hack
# then hack: 
# 启动之后把镜像（注意有两个 image: 需要修改） dockerhub.bigquant.ai:5000/devops/artifactory-pro:7.10.6 -> dockerhub.bigquant.ai:5000/devops/artifactory-pro:7.10.6-hack-helm 

# 备注：
# 容器内 JVM 路径：/opt/jfrog/artifactory/app/third-party/java/bin/java
# 启动 war 包所在的路径：/opt/jfrog/artifactory/app/artifactory/tomcat
hack 一下激活相关的几个 class。重新打包就好，hack 好的文件在代码中
```

参考密钥：

```text
eyJhcnRpZmFjdG9yeSI6eyJpZCI6IiIsIm93bmVyIjoicjRwMyIsInZhbGlkRnJvbSI6MTYzNzY1MDc0MTM0MSwiZXhwaXJlcyI6NDc5MzMyNDM0MTM0MCwidHlwZSI6IkVOVEVSUFJJU0VfUExVUyIsInRyaWFsIjpmYWxzZSwicHJvcGVydGllcyI6e319fQ==
```
