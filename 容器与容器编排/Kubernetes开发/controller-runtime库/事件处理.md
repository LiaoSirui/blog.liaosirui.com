...
调谐遵循的是 level-driven（状态驱动） 而非 edge-driven（事件驱动） 的理念

```bash
事件(Event) → 工作队列(WorkQueue) → 调谐(Reconcile)
                  ↑ 去重/合并

```

多个事件可能被合并为一次调谐。WorkQueue 按照资源的 `namespace/name` 进行去重，如果短时间内同一个资源产生了多个事件，队列中只会保留一个条目。

触发调谐的来源有多种：

```bash
// ① Watch 到的资源变更事件
ctrl.Watch(source.Kind(cache, &v1.Pod{}), handler)

// ② 调谐函数主动要求重新入队
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
    // 返回 RequeueAfter → 定时再次调谐（无需新事件）
    return ctrl.Result{RequeueAfter: 30 * time.Second}, nil
    
    // 返回 Requeue: true → 立即重新入队
    return ctrl.Result{Requeue: true}, nil
    
    // 返回 error → 自动按退避策略重新入队
    return ctrl.Result{}, err
}

// ③ 周期性 resync（informer 定期全量重列）
...
时间线:
t0: 事件到达 → key 入队
t1: Reconcile 开始 → key 出队，标记为 "processing"
t2: 处理期间新事件 → key 重新入队（但不会立刻处理！）
t3: Reconcile 结束 → 调用 queue.Done(key)
t4: 此时才开始处理队列中的下一个 key（即 t2 入队的那个）
```

流程

```bash
事件A ──→ dirty ✓, processing ✗ ──→ 直接入 queue
                                         │
                                    worker 取出处理
                                    (processing ✓, dirty ✗)
                                         │
事件B ──→ dirty ✓, processing ✓ ──→ 只标记dirty，不入queue
事件C ──→ dirty ✓（已存在）      ──→ 跳过
                                         │
                                    Reconcile 结束
                                    调用 Done()
...
//此处定义了针对不同事件的处理接口，可以通过实现此接口完成扩展业务逻辑
...
可能会遇到 CR -> Deployment -> Pod 的逻辑，即由CR 创建deployment，最终落入pod。这时，不仅需要监听 CR 的变化， deployment 以及 pod 的变化也需要关注，这就意味着在 reconciler 中也需要根据 deployment 变化进行相应的处理
...
看`NewControllerManagedBy()`的源码可以发现`pkg\builder\controller.go` 中实现了 **构建者模式**，`controller-runtime` 提供了一个 `builder` 方便进行配置。其中监听资源的有 `For()` `Owns()` 和`Watch()` ,`For()` `Owns()`都是基于`Watch()`实现，只不过是入队列前的 `eventHandler` 不同
