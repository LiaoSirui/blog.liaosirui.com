...
- `Reallocated_Sector_Ct` (05)：已重映射的坏扇区数，非 0 即代表存在物理坏道

- ``Current_Pending_Sector` (C5)：待映射的疑似坏扇区，数值增长需警惕
- ``Offline_Uncorrectable` (C6)：离线无法修复的扇区，直接反映坏道问题
- `UDMA_CRC_Error_Count` (C7)
- `Reported_Uncorrect` (BB)
